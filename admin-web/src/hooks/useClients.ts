...
export const useClients = () => {
  ...
  const update = useMutation<IClient, Error, { id: string; payload: Partial<IClient> }>({
...
const reactivate = useMutation<IClient, Error, { clientId: string; extendMonths: number }>({
  mutationFn: ({ clientId, extendMonths }) => clientApi.reactivateClient(clientId, extendMonths),
  onSuccess: () => queryClient.invalidateQueries({ queryKey: ['clients'] }),
});
