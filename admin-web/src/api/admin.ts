...
export const createClient = async (payload: Partial<IClient>): Promise<IClient> => {
...
export const updateClient = async (clientId: string, payload: Partial<IClient>): Promise<IClient> => {
...
export const reactivateClient = async (clientId: string, extendMonths: number) => {
...
export const getPlans = async (): Promise<IPlan[]> => {
  const data = await fetcher("/plans");
  return data.plans;
};
...
export const updatePlan = async (id: string, payload: Partial<IPlan>) => {
  const data = await fetcher(`/plans/${id}`, {
    method: "PUT",
    data: payload,
  });
  return data.plan;
