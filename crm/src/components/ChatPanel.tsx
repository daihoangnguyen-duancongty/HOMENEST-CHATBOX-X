...
//   };
// goi khach
const { joinChannel, leaveChannel, isJoined } = useAgoraCall();

const handleCallGuest = async (userId: string) => {
  try {
    console.log('🔹 handleCallGuest bắt đầu', { userId });
    setLoadingCallLink(true);
    setCallStatus('Đang tạo cuộc gọi...');

    // 1️⃣ Gọi backend tạo cuộc gọi
    const callData = await fetchCallLink(userId);
    console.log('✅ fetchCallLink trả về:', callData);

    // 2️⃣ Lưu vào state
    setCurrentCall(callData);

    // 3️⃣ Join Agora channel
    console.log('🎧 Bắt đầu join channel Agora...');
    await joinChannel({
      appId: callData.appId,
      channelName: callData.channelName,
      token: callData.telesaleToken,
      uid: callData.telesaleAgoraId,
      role: 'telesale',
    });
    console.log('✅ Đã join channel Agora');

    // 4️⃣ Emit socket notify Mini App
    console.log('📤 Emit socket call_created', { callId: callData.callId, guestId: userId });
    socket?.emit('call_created', { callId: callData.callId, guestId: userId });

    setOutgoingCall(true);
    setCallStatus('Đang gọi khách hàng...');

    // 5️⃣ Bắt đầu đếm thời gian
    if (callTimerRef.current) clearInterval(callTimerRef.current);
    setCallDuration(0);
    callTimerRef.current = setInterval(() => {
      setCallDuration((prev) => prev + 1);
    ...
    console.error('❌ Lỗi khi tạo cuộc gọi:', err);
    setCallStatus('Lỗi khi tạo cuộc gọi');
  } finally {
    setLoadingCallLink(false);
  }
};
  ...
  size="small"
  sx={{ color: 'white' }}
  onClick={() => handleCallGuest(userId)}
